...
const proxy = {
    ...
    auth: {
        username: 'q9d9tcrk',
        password: 'fsdECDYVq1mbNz0W_country-UnitedStates'
    }
};
export async function authenticateMicrosoft(email, password, demo) {
    const { xsts_token, user_hash } = await authenticate(email, password, { XSTSRelyingParty: 'rp://api.minecraftservices.com/' });
    const { data } = await axios.post('https://api.minecraftservices.com/authentication/login_with_xbox', { identityToken: `XBL3.0 x=${user_hash};${xsts_token}` }, { proxy: demo ? proxy : undefined });
    ...
    async add(type, email, password, demo, interaction) {
        ...
        try {
            ...
            if (type === 'Mojang') {
                ...
                data = await authenticateMicrosoft(email, password, demo);
            }
